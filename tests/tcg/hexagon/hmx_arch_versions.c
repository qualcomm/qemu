...
/*                                                                     */
/* v71+ (unsigned): use shape=2 (relu) with high output offset to      */
/*   reliably produce non-zero from the positive accumulator.          */
/* pre-v71 (signed): use shape=2 (relu) with zero output offset so     */
...
static void test_weight_signedness(uint8_t *base, unsigned int hex_ver)
{
    ...
    if (hex_ver >= 0x71) {
        /*
         * v71+: unsigned weight (0xFE=254), acc=128*254=+32512.
         * Use out_bias=0 which encodes to maximum positive output
         * offset, ensuring the positive accumulator reliably produces
         * non-zero through the convert pipeline on all targets.
         */
        setup_fxp_bias(bias_area, pack_fxp_bias(0, 0, 2, 0x400, 0));
    } else {
        /*
         * pre-v71: signed weight (0xFE=-2), acc=128*(-2)=-256.
         * Use out_bias=0xFF0 which encodes to zero output offset
         * (the encoding inverts: (~0xFF0 >> 4) & 0xFF == 0x00).
         * With zero offset, relu clamps the negative accumulator
         * to exactly zero.
         */
    ...
    if (hex_ver >= 0x71) {
        /* v71+: unsigned weight (0xFE=254), MAC=32512, relu passes */
        if (val == 0) {
            printf("ERROR: v71+ unsigned weight should produce non-zero, "
                   "got 0\n");
            err++;
        }
    } else {
        /* pre-v71: signed weight (0xFE=-2), MAC=-256, relu -> 0 */
        check_val(val, 0, "pre-v71 signed weight should relu to 0");
    }
}
