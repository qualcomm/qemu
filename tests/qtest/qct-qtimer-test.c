...
/*
 * Exercise the CNTACR-gated denial paths in the view region. A denied
 * access makes the device return MEMTX_ACCESS_ERROR, which qtest cannot
 * observe directly: the qtest read/write commands discard the MemTxResult.
 * It is still observable indirectly, because memory_region_dispatch_read()
 * zeroes the destination before calling the device and the device leaves it
 * untouched when it denies, so a denied read yields 0 and a denied write is
 * dropped. These assertions therefore pin down the side effects rather than
 * the bus error itself.
 */
...
/*
 * Each frame drives its own L2VIC input: hardware wires frame N to
 * l2vic_int_src[QTIMER_L2VIC_IRQ_BASE + N]. Intercepting the L2VIC inputs
 * reports the line number the frame is actually connected to, so a frame
 * firing on the wrong input fails here. Runs last: the interception stays in
 * effect for the rest of the VM's life.
 */
