...
similar to other architectures'.  The ``trap0`` instruction can activate
these semihosting calls so that the guest software can access the host console
and filesystem.  Semihosting is not yet implemented in QEMU hexagon.
...
`minivm <https://github.com/quic/hexagonMVM>`__ is a reference implementation
