...
def one_kconf_entry(name: str):
    return f'''
#ifdef COMPILING_PER_TARGET
 #ifdef CONFIG_{name}
  KCONF_ENABLED({name}, {name.lower()})
 #else
  KCONF_DISABLED({name}, {name.lower()})
 #endif
#else
 KCONF_SYMBOL({name}, {name.lower()})
#endif
    '''.strip()

def print_kconf_header(out: T.TextIO, names: T.List[str]):
    entries = '\n'.join(map(one_kconf_entry, names))
    print(f'''
/*
 * Autogenerated by scripts/minikconf.py.
 * Do not edit, your changes will be lost.
 */

#ifdef COMPILING_PER_TARGET
 #ifndef KCONF_ENABLED
  #error KCONF_ENABLED macro is not defined
 #endif
 #ifndef KCONF_DISABLED
  #error KCONF_DISABLED macro is not defined
 #endif
#else
 #ifndef KCONF_SYMBOL
  #error KCONF_SYMBOL macro is not defined
 #endif
#endif
...
def main() -> None:
    ...
    if genkconfheader:
        with open(argv[1], 'wt', encoding='utf-8') as out:
            print_kconf_header(out, sorted(config.keys()))
    else:
