 ...
 *
 * The reference plugin defers pipeline aging to commit_regs time.
 ...
 *
 * Mirrors the reference plugin's commit_regs + commit_mem:
 ...
 *
 * This implements the logic from hmx_act_paramaters() in the reference
 * simulator (hmx_int_ops.c). Called from hmx_act_load to pre-compute
                                    ...
                                    int act_fmt, int act_mod, int act_type)
{
    ...
    /*
     * Channel range.
     * Reference: hmx_act_paramaters() in hmx_int_ops.c lines 2782-2877.
     */
    /*
    ...
    if (hmx->group_conv) {
         ...
         * Reference: fCL1_1 takes uint8_t (count_leading_ones_1).
         */
     ...
     * Reference: hmx_act_paramaters() in hmx_int_ops.c lines 2788-2791.
     */
    if (is_flt) {
        hmx->tile_x_mask &= ~1;
        hmx->tile_y_mask &= ~1;
     ...
     * process.  Reference: hmx_act_paramaters line 2875 sets blocks=0.
     */
 ...
 * Reference: hmx_unpack_sm_from_byte computes (((in >> 7)) & 0x7F) ^ in.
 ...
 * Reference: hmx_unpack_nibble_from_byte uses hmx_sign_extend_to_byte.
 */
 ...
 * Reference: hmx_unpack_crumb_from_byte uses hmx_sign_extend_to_byte.
 */
 ...
 *
 * Reference: hmx_calc_crouton_range() in hmx_int_ops.c.
 */
                             ...
                             uint32_t params)
{
    ...
    /*
     * Weight address range from Rt.  The reference (system_ext_hmx.c)
     ...
     * Reference: hmx_wgt_paramaters() sets wgt_negate = 0 when !is_flt.
     */
    int negate = 1;

    /*
     * X-dimension tap parameters (determined by weight modifier).
     * Reference: hmx_wgt_paramaters() in hmx_int_ops.c.
     */
    uint32_t x_start = 0;
    uint32_t x_stop = 0;
    int x_dilate = 0;
     ...
     * hmx_wgt_paramaters in hmx_int_ops.c).
     */
     ...
     *
     * Reference: hmx_multiply_old() in hmx_int_ops.c.
     ...
     * hmx_multiply_full_math() in hmx_int_ops.c.
     */
     ...
     * Reference: hmx_multiply_full_math() loops over wgt_deep_idx
                            ...
                            uint32_t params)
{
     ...
     * Reference: hmx_wgt_paramaters() sets wgt_negate = (Rs[5]) << 15
                           ...
                           uint32_t params)
{
     ...
     * Reference: hmx_ld_act() loads 2nd block from base + dY when
        ...
        if (need_second) {
            ...
            for (int s = HMX_SPATIAL_DIM_FP - 1; s >= 0; s--) {
                ...
                for (int c = HMX_INPUT_CHANNELS - 1; c >= 0; c--) {
                    ...
                    uint8_t f8 = hmx->act_buffer[buf_off];
                    uint16_t f16;
                    if (f8 == 0x80) {
                        f16 = 0xFE00;
                    } else {
...
/*
 * Convert equation ported from reference coproc simulator
 * (hmx_int_ops.c: hmx_u8_cvt and helpers)
 *
...
/*
 * hmx_u8_cvt: byte convert - ported from reference simulator.
 *
 * Parameters:
 ...
 * value.  Uses rounding (not output bias) per reference hmx_u16_cvt.
 *
 * Reference: hmx_int_ops.c: hmx_u16_cvt() lines 284-327
 */
static uint32_t hmx_u16_cvt(int64_t acc_hl, int64_t acc_ll,
                              int32_t bias32, int16_t exp,
                              int16_t zeroing, int16_t sig,
 ...
 *
 * Reference: hmx_int_ops.c: hmx_u16x16_cvt() lines 330-377
 */
 ...
 * Reference: hmx_cvt_out_lo / hmx_cvt_out_hi in hmx_int_ops.c
 *
 ...
 *   [13:12] bias_sel
 *
 * Reference: hmx_acc_convert() -> hmx_8x8_convert_body() in
 * hmx_int_ops.c
 */
                             ...
                             uint32_t cur_pc)
{
     ...
     *
     * The reference plugin sets cvt_advance based on the LAST cvt_rs
 ...
 *
 * Reference: hmx_acc_convert() -> hmx_16x8_convert_body() in
 * hmx_int_ops.c
 */
static void hmx_fxp_convert_2x1(HmxState *hmx, int acc_set, int relu,
                                  int bias_set, int fb_dst, uint32_t cur_pc)
{
    HmxAccFxp *acc = &hmx->acc[acc_set & 1].fxp_primary;
    ...
    for (int s = 0; s < HMX_SPATIAL_DIM_FXP; s += 2) {
        for (o = 0; o < HMX_OUTPUT_CHANNELS; o++) {
            ...
            uint32_t result = hmx_u16_cvt(
                ...
                bias_shape[o], bias_scale[o], bias_rnd[o],
                /* sat */ !relu, /* legacy */ 0);
 ...
 *
 * Reference: hmx_acc_convert() -> hmx_16x16_convert_body() in
 * hmx_int_ops.c
 */
                                  ...
                                  int bias_set, int fb_dst, uint32_t cur_pc)
{
    ...
    for (int s = 0; s < HMX_SPATIAL_DIM_FXP; s += 2) {
        for (int o = 0; o < HMX_OUTPUT_CHANNELS; o += 2) {
             ...
             *           (sig_lo << 1) | siglsb_lo
             * Reference: hmx_16x16_convert_body lines 929-930
             */
             ...
             *   out_bias = ((bias1_hi << 12) | bias_lo) << 2
             * Reference: lines 929
             */
             ...
             * in feedback mode.  Reference: lines 951-953.
             */
            if (!fb_dst) {
                cvt_out->data[s][o + 1] = 0;
                cvt_out->data[s + 1][o + 1] = 0;
            }
        }
    }
}
                               ...
                               uint32_t params)
{
    ...
    if (fmt == HMX_CVT_FMT_UH) {
         ...
         * Reference: hmx_16x8_convert_body() in hmx_int_ops.c
         */
        for (int s = s_start; s < s_end; s += 2) {
            for (int o = 0; o < HMX_OUTPUT_CHANNELS; o++) {
                int64_t acc_ll = (int64_t)acc->data[s][o];
                int64_t acc_hl = (int64_t)acc->data[s + 1][o];
                uint16_t rnd = (hmx->bias_raw[bias_set][o] >> 22) & 1;

                uint32_t result = hmx_u16_cvt(
                    acc_hl, acc_ll, bias_input[o], bias_exp[o],
                    bias_shape[o], bias_scale[o], rnd,
                    /* sat */ !relu, /* legacy */ 1);
         ...
         * bias registers.
         * Reference: hmx_16x16_convert_body() in hmx_int_ops.c
         */
        for (int s = s_start; s < s_end; s += 2) {
            for (int o = 0; o < HMX_OUTPUT_CHANNELS; o += 2) {
                uint64_t raw_lo = hmx->bias_raw[bias_set][o];
    ...
    for (int s = s_start; s < s_end; s++) {
        ...
        if (split_offset != 0 && split_inc != 0) {
            uint32_t sa = is_cm ? (uint32_t)(s << 5) :
                          (uint32_t)(((s >> 2) << 7) | (s & 3));
            uint32_t s_addr = sa & split_mask;
            uint32_t o_addr = sa & ~split_mask & spatial_mask;
            uint32_t s_pos = s_addr / split_inc;
    ...
    /* Clear accumulator and flip acc set (reference: ext_hmx.c line 514) */
    if (!retain) {
        for (int s = s_start; s < s_end; s++) {
 ...
 * Results are stored as F8 values shifted left by 4 in cvt_fp.
 */
/*
                            ...
                            int relu, int bias_sel, int maxnorm)
{
    ...
    for (int o = 0; o < HMX_OUTPUT_CHANNELS; o++) {
        ...
        for (int s = 0; s < HMX_SPATIAL_DIM_FP; s++) {
             ...
             * Reference: shape=3 means |x|. The abs is achieved by
 ...
 * then fMX_CVT() which runs hmx_acc_convert().
 *
 * Rs bitfield (hmx_cvt_rs_reg_t):
 ...
 * reference write_x_row in ext_hmx.c.
 */
                            ...
                            uint32_t params)
{
    ...
    if (fmt == HMX_CVTST_F8) {
        /*
         * FP8 store: values pre-computed in cvt_fp, shifted << 4.
         * Rs[0] selects odd (1) or even (0) spatial positions within
         * the crouton.  FP has 32 spatial positions that map to
         * crouton indices: even (0,2,4,...) or odd (1,3,5,...).
         */
        HmxCvtStateFp *cvt = &hmx->cvt_fp[age];
        int odd_spatial = rs & 1;
        int s, o;

        for (s = 0; s < HMX_SPATIAL_DIM_FP; s++) {
            for (o = 0; o < HMX_OUTPUT_CHANNELS; o++) {
                uint8_t f8 = (cvt->data[s][o] >> 4) & 0xFF;
                int crouton_s = s * 2 + odd_spatial;
                int offset = hmx_act_offset_sm(crouton_s, o);
                cpu_stb_data_ra(env, base_addr + offset, f8, ra);
            }
        }
        return;
    }
     ...
     * rather than memory position, matching the reference write_x_row.
     */
     ...
     * Reference: hmx_cvt_mem_parameters in hmx_int_ops.c
     */
