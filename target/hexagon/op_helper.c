...
void HELPER(dmstart)(CPUHexagonState *env, uint32_t desc_va)
{
    ...
    while (va != 0) {
        if (++iteration > 1024) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "%s: descriptor list exceeds maximum depth\n",
                          __func__);
            break;
        }
        for (i = 0; i < 4; i++) {
            desc[i] = cpu_ldl_data_ra(env, va + i * 4, ra);
        }
        ...
        for (off = 0; off < length; off++) {
            uint8_t byte = cpu_ldub_data_ra(env, src + off, ra);
            cpu_stb_data_ra(env, dst + off, byte, ra);
        }

        /* Mark descriptor as DONE (bit 31 of word[1]) */
        desc[1] |= 0x80000000;
        cpu_stl_data_ra(env, va + 4, desc[1], ra);
...
static inline int cmpgt_hf_wrap(int16_t a, int16_t b)
{
    ...
    uint32_t sa = (ha >> 15) & 1;
    uint32_t ea = (ha >> 10) & 0x1f;
    uint32_t ma = ha & 0x3ff;
    uint32_t sb = (hb >> 15) & 1;
    uint32_t eb = (hb >> 10) & 0x1f;
    uint32_t mb = hb & 0x3ff;
...
#ifndef fCVI_VX_NO_TMP_LD
#define fCVI_VX_NO_TMP_LD()
#endif

#ifndef fCMPGT_BF
#define fCMPGT_BF(A, B) cmpgt_sf_wrap(((int)(A)) << 16, ((int)(B)) << 16)
#endif
