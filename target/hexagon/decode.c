...
static void decode_fill_cycle_estimates(Packet *pkt)
{
    for (int i = 0; i < pkt->num_insns; i++) {
        switch (pkt->insn[i].opcode) {

#define DECODE_OPINFO(TAG, BEH) case TAG: BEH; break;
#define DECODE_CYCLES(C) pkt->insn[i].cycles = C
                  ...
                  Packet *pkt, bool disas_only, uint32_t rev, uint32_t pc)
{
    ...
    decode_fill_cycle_estimates(pkt);
