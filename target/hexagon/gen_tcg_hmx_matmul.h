                                      ...
                                      int wei_type, int wei_mod)
{
     ...
     * reads state from HmxState at runtime — matching hexagon-sim behavior
    ...
    /*
     * Weight signedness: on v71+, byte weights are unsigned.
     * This is a translation-time decision based on the CPU model.
     */
    int wei_unsigned = (wei_type == HMX_WEI_B) &&
                       (ctx->hex_def->hex_version >= HEX_VER_V71);
     ...
     * treated as zero (reference: hmx_ld_wgt marks them invalid).
     */
     ...
     * Reference: hmx_mult_inner checks wgt_cache[idx][0].valid and
     ...
     * accumulator.  Reference: hmx_multiply_full_math() loops over
