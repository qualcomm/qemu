...
regre = re.compile(r"((?<!DUP)[MNORCPQXSGVZA])([stuvwxyzdefg]+)([.]?[LlHh]?)(\d+S?)[.]?(qf16|qf32|)")
...
#
# TODO: we need registers for system emulation
class Scalar:
...
class Hvx:
    ...
    def is_qfloat(self):
        return False # "no" by default

    def from_subtype(self, subtype):
        if subtype == "":
            return self
        elif subtype in ("qf16", "qf32"):
            from copy import deepcopy
            new = deepcopy(self)
            new.is_qfloat = lambda: True
            return new
        raise Exception(f"unknown subtype '{subtype}' in Hvx register class")
...
class GprDest(Register, Single, Dest):
    ...
    def gen_write(self, f, tag):
        if is_predicated(tag):
            f.write(code_fmt(f"""\
                gen_check_reg_write(ctx, {self.regtype}{self.regid}N);
            """))
...
class PairDest(Register, Pair, Dest):
    ...
    def gen_write(self, f, tag):
        ...
        if is_predicated(tag):
            f.write(code_fmt(f"""\
                gen_check_reg_write_pair(ctx, {self.regtype}{self.regid}N);
            """))
    def analyze_write(self, f, tag, regno):
        predicated = "true" if is_predicated(tag) else "false"
        f.write(code_fmt(f"""\
...
class VRegDest(Hvx, Register, Single, Dest):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
...
class VRegSource(Hvx, Register, Single, OldSource):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
...
class VRegNewSource(Hvx, Register, Single, NewSource):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        if skip_qemu_helper(tag):
            ...
            """))
        # No need for gen_clear_extended(); new VRegs are automatically cleaned
    def helper_hvx_desc(self, f):
        f.write(code_fmt(f"""\
            /* {self.reg_tcg()} is *(MMVector *)({self.helper_arg_name()}) */
        """))
...
class VRegReadWrite(Hvx, Register, Single, ReadWrite):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class VRegTmp(Hvx, Register, Single, ReadWrite):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
...
class VRegPairDest(Hvx, Register, Pair, Dest):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class VRegPairSource(Hvx, Register, Pair, OldSource):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class VRegPairReadWrite(Hvx, Register, Pair, ReadWrite):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class QRegDest(Hvx, Register, Dest):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class QRegSource(Hvx, Register, OldSource):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class QRegReadWrite(Hvx, Register, ReadWrite):
    def decl_tcg(self, f, tag, regno):
        self.decl_reg_num(f, regno)
        f.write(code_fmt(f"""\
            const intptr_t {self.hvx_off()} =
...
class GuestDest(GuestRegister, Single, Dest):
    ...
    def gen_write(self, f, tag):
        f.write(code_fmt(f"""\
            gen_greg_write(ctx, {self.reg_num}, {self.reg_tcg()});
        """))
    def analyze_write(self, f, tag, regno):
        f.write(code_fmt(f"""\
...
class GuestPairDest(GuestRegister, Pair, Dest):
    ...
    def gen_write(self, f, tag):
        f.write(code_fmt(f"""\
            gen_greg_write_pair(ctx, {self.reg_num}, {self.reg_tcg()});
        """))
    def analyze_write(self, f, tag, regno):
        f.write(code_fmt(f"""\
...
class SystemDest(Register, Single, Dest):
    ...
    def gen_write(self, f, tag):
        f.write(code_fmt(f"""\
            gen_sreg_write(ctx, {self.reg_num}, {self.reg_tcg()});
        """))
    def analyze_write(self, f, tag, regno):
        f.write(code_fmt(f"""\
...
class SystemPairDest(Register, Pair, Dest):
    ...
    def gen_write(self, f, tag):
        f.write(code_fmt(f"""\
            gen_sreg_write_pair(ctx, {self.reg_num}, {self.reg_tcg()});
        """))
    def analyze_write(self, f, tag, regno):
        f.write(code_fmt(f"""\
