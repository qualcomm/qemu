...
static inline uint32_t get_dst_widthoffset(HEXAGON_DmaDescriptor2D_t *desc, dma_decoded_descriptor_t *desc_state) {
	if (desc_state->wide) {
		...
		if (desc_state->dwo) {
			...
			return desc->srcWidthOffset;  // FIXME: eventually <<2 or >>2 depending on transform field
		}
	}
}
...
static uint32_t ARCH_FUNCTION(start_dma)(dma_t *dma, uint32_t new_dma)
{
    ...
    if (!desc_state->pa)
    {
        ...
        udma_ctx->exception_va = new_dma;
				/* This may pass garbage to the dma_callback in the info struct, depending on the exception type */
				/* xact ID == -1 (not stepped); pause == 0; exception == 1 */
...
static void preload_buffers(dma_t *dma, uint32_t new)
{
    ...
    if (dma_type == 0) {
        while (desc_addr) {
            ...
            hexagon_touch_memory(env, dst_va, length, MMU_DATA_LOAD | MMU_DATA_STORE);
            hexagon_touch_memory(env, src_va, length, MMU_DATA_LOAD | MMU_DATA_STORE);
        ...
        while (desc_addr) {
            ...
            hexagon_touch_memory(env, dst_va, size, MMU_DATA_LOAD | MMU_DATA_STORE);
            hexagon_touch_memory(env, src_va, size, MMU_DATA_LOAD | MMU_DATA_STORE);
