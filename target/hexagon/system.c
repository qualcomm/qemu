...
#ifndef CONFIG_USER_ONLY
static void register_error_exception(thread_t *thread, size4u_t error_code,
                                     size4u_t badva0, size4u_t badva1,
                                     size4u_t bvs, size4u_t bv0, size4u_t bv1,
                                     size4u_t slotmask);
static void register_exception_info(thread_t *thread, size4u_t type,
                                    size4u_t cause, size4u_t badva0,
                                    size4u_t badva1, size4u_t bvs, size4u_t bv0,
                                    size4u_t bv1, size4u_t elr, size4u_t diag,
                                    size4u_t de_slotmask);

#endif

#include "string.h"

#define TLBGUESSIDX(VA) \
    (((VA >> 12) ^ (VA >> 22)) & (MAX_TLB_GUESS_ENTRIES - 1))
...
{
    int violation = 0;
    uint32_t vwctrl = thread->t_sreg[HEX_SREG_VWCTRL];
    ...
    if (enabled == 0) {
        ...
        win_lo_addr =
            get_vtcm_base(thread) + (GET_FIELD(VWCTRL_LOWOFFSET, vwctrl) << 12);
        win_hi_addr = get_vtcm_base(thread) +
                      ((GET_FIELD(VWCTRL_HIOFFSET, vwctrl) << 12) | 0xfff);
        violation = (pa < win_lo_addr || pa > win_hi_addr);
    }
    if (violation) {
        SYSVERWARN(
            "VTCM access window violation: violation=%d enabled=%d paddr=0x%lx "
            "vwctrl_low=0x%lx vwctrl_hi=0x%lx lower:%d higher:%d",
            violation, enabled, pa, win_lo_addr, win_hi_addr,
            (pa < win_lo_addr), (pa > win_hi_addr));
    }
    return violation;
}
...
{
    target_ulong ssr = thread->t_sreg[HEX_SREG_SSR];
     ...
     */
    if ((einfo->type == EXCEPT_TYPE_PRECISE) &&
        (einfo->cause < PRECISE_CAUSE_DOUBLE_EXCEPT))
        register_double_exception = 0;

    if (register_double_exception) {
        warn("Double Exception (from: type=%x cause=%x elr=%x)", einfo->type,
             einfo->cause, einfo->elr);
        register_exception_info(
            thread, EXCEPT_TYPE_PRECISE, PRECISE_CAUSE_DOUBLE_EXCEPT,
            thread->Regs[REG_BADVA0], thread->Regs[REG_BADVA1],
            GET_SSR_FIELD(SSR_BVS, ssr), GET_SSR_FIELD(SSR_V0, ssr),
            GET_SSR_FIELD(SSR_V1, ssr), einfo->elr, einfo->cause,
            einfo->de_slotmask);
    } else {
        warn("Registering exception info: type=%x cause=%x elr=%x badva0=%x "
             "badva1=%x bvs=%d",
             einfo->type, einfo->cause, einfo->elr, einfo->badva0,
             einfo->badva1, einfo->bvs);
        register_exception_info(thread, einfo->type, einfo->cause,
                                einfo->badva0, einfo->badva1, einfo->bvs,
                                einfo->bv0, einfo->bv1, einfo->elr,
                                hexagon_globalreg_read(
                                    env_archcpu(thread)->globalregs,
                                    HEX_SREG_DIAG), 0);
    }
}

static void fill_einfo_ldst(thread_t *thread, hex_exception_info *einfo,
...
static inline void fill_einfo_ldsterror(thread_t *thread,
                                        hex_exception_info *einfo,
                                        size4u_t slot, size4u_t cause,
                                        size4u_t va)
{
...
static void sys_check_vwctrl(thread_t *thread, int slot, vaddr_t va, paddr_t pa)
{
     ...
     * accessible.
     */
    if (thread->t_sreg[HEX_SREG_VWCTRL] == 0x8fff0000) {
    ...
    if (is_vtcm_acc_window_violation(thread, pa)) {
        ...
        if (!(sys_in_monitor_mode(thread))) {
            hex_exception_info einfo;
            fill_einfo_ldsterror(thread, &einfo, slot,
                                 HEX_CAUSE_VWCTRL_WINDOW_MISS, va);
...
paddr_t mem_init_access_unaligned(thread_t *thread, int slot, size4u_t vaddr,
                                  size4u_t realvaddr, int size,
                                  enum mem_access_types mtype,
                                  int type_for_xlate)
{
    paddr_t ret;
    mem_access_info_t *maptr = &thread->mem_access[slot];
...
#ifndef CONFIG_USER_ONLY
static int is_du_badva_affecting_exception(int type, int cause)
{
...
static void raise_coproc_ldst_exception(thread_t *env, size4u_t de_slotmask,
                                        target_ulong PC)

{
    CPUState *cs = env_cpu(env);
    size4u_t slot = (de_slotmask & 0x1) ? 0 : 1;
    raise_perm_exception(cs, thread->einfo.badva1, slot, MMU_DATA_LOAD,
                         thread->einfo.type);
...
{
#ifdef VERIFICATION
    warn("Oldtype=%d oldcause=0x%x newtype=%d newcause=%x de_slotmask=%x "
         "diag=%x",
         thread->einfo.type, thread->einfo.cause, type, cause, de_slotmask,
         diag);
#endif
    warn("Oldtype=%d oldcause=0x%x newtype=%d newcause=%x de_slotmask=%x "
         "diag=%x",
    ...
    if ((EXCEPTION_DETECTED) &&
        ...
        warn("FP Exception has higher priority than multi write / bad "
             "cacheability");
    } else if ((EXCEPTION_DETECTED) &&
               (thread->einfo.type == EXCEPT_TYPE_TLB_MISS_RW) &&
               ((type == EXCEPT_TYPE_PRECISE) &&
                ((cause == 0x26) || (cause == 0x27) || (cause == 0x28) ||
                 (cause == 0x29)))) {
        warn("Footnote in v2 System Architecture Spec 5.1 says: TLB miss RW "
             "has higher priority than multi write / bad cacheability");
    }

    else if ((EXCEPTION_DETECTED) &&
             (thread->einfo.cause == PRECISE_CAUSE_BIU_PRECISE) &&
             ((cause == PRECISE_CAUSE_REG_WRITE_CONFLICT) ||
    ...
    } else if ((type == thread->einfo.type) && (cause < thread->einfo.cause)) {
        thread->einfo.cause = cause;
        thread->einfo.badva0 = badva0;
        thread->einfo.badva1 = badva1;
        thread->einfo.bvs = bvs;
        ...
        thread->einfo.diag = diag;
               ...
               (cause == PRECISE_CAUSE_DOUBLE_EXCEPT)) {
        if ((de_slotmask == 0) || (thread->einfo.de_slotmask < de_slotmask)) {
            if (diag < thread->einfo.diag) {
                warn("Picking better proiroty root exception cause for DIAG: "
                     "0x%x",
                     diag);
                thread->einfo.diag = diag;
            } else {
            ...
            warn("Trying to avoid slot0 DE in the presence of a slot1 DE, not "
                 "setting slot0 DIAG of 0x%x",
                 diag);
        }
    } else {
                                     ...
                                     size4u_t slotmask)
{
    ...
    if ((error_code > PRECISE_CAUSE_DOUBLE_EXCEPT) &&
        ...
        register_exception_info(
            ...
            thread->t_sreg[HEX_SREG_BADVA0],
            thread->t_sreg[HEX_SREG_BADVA1],
            GET_SSR_FIELD(SSR_BVS, ssr), GET_SSR_FIELD(SSR_V0, ssr),
