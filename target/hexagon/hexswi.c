...
static void coredump(CPUHexagonState *env)
{
    ...
    case HEX_CAUSE_PRIV_NO_UREAD:
        fprintf(f, "0x%x, Privilege violation:"
                " user read permission @ 0x%x",
                HEX_CAUSE_PRIV_NO_UREAD,
                arch_get_system_reg(env, HEX_SREG_BADVA));
        break;
    case HEX_CAUSE_PRIV_NO_UWRITE:
        fprintf(f, "0x%x, Privilege violation:"
                " user write permission @ 0x%x",
                HEX_CAUSE_PRIV_NO_UWRITE,
                arch_get_system_reg(env, HEX_SREG_BADVA));
        break;
    case HEX_CAUSE_COPROC_LDST:
        fprintf(f, "0x%x, Coprocessor VMEM address error @ 0x%x",
...
void hexagon_vmrte(CPUHexagonState *env)
{
    ...
    qemu_log_mask(CPU_LOG_INT,
                  "hexagon_vmrte: GSR=0x%x SSR=0x%x GELR=0x"
                  TARGET_FMT_lx "\n",
                  gsr, old_ssr, gelr);
