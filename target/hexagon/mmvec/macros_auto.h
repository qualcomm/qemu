...
#define fDUMPQ(STR,REG) do { printf(STR ":" #REG ": 0x%016llx ",REG.ud[0]); } while (0)
#define fUSE_LOOKUP_ADDRESS_BY_REV(PROC) PROC->arch_proc_options->HVX_USE_FULL_VA
...
#define fGETQFEXT_BIT(REG,BITNO) (0x1 & (REG.ext[(BITNO)/4] >> ((BITNO) % 4)))
...
#define fSETQFEXT_BIT(REG,BITNO,VAL) do { size4u_t __TMP = (VAL); REG.ext[(BITNO)/4] &= ~((1) << ((BITNO) % 4)); REG.ext[(BITNO)/4] |= (((__TMP) & (1)) << ((BITNO) % 4)); } while (0)
...
#define fVFETCH_AL(EA) { fV_AL_CHECK(EA,fVECSIZE()-1); mem_fetch_vector(thread, insn, EA&~(fVECSIZE()-1), insn->slot, fVECSIZE()); }
#define fLOADMMV_AL(EA,ALIGNMENT,LEN,DST) { fV_AL_CHECK(EA,ALIGNMENT-1); thread->last_pkt->double_access_vec = 0; mem_load_vector_oddva(thread, insn, EA&~(ALIGNMENT-1), EA, insn->slot, LEN, &DST.ub[0], LEN, fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); }
#ifdef QEMU_GENERATE
#define fLOADMMV(EA, DST) gen_vreg_load(ctx, DST##_off, EA, true)
#else
#define fLOADMMV(EA,DST) fLOADMMV_AL(EA,fVECSIZE(),fVECSIZE(),DST)
#endif
#define fLOADMMZ(EA,DST) { mmvector_t load_vec; fV_AL_CHECK(EA,fVECSIZE()-1); mem_load_vector_oddva(thread, insn, EA&~(fVECSIZE()-1), EA, insn->slot, fVECSIZE(), &load_vec.ub[0], fVECSIZE(), fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); int idx = (EA & 0x80)>0; DST.v[idx] = load_vec; }
...
#define fELSE_CANCELZ() else { if (thread->last_pkt) { thread->mem_access[insn->slot].dropped_z = 1; thread->last_pkt->ext_slot_cancelled |= (1<<insn->slot); } }
...
#define fLOADMMVU_AL(EA,ALIGNMENT,LEN,DST) { size4u_t size2 = (EA)&(ALIGNMENT-1); size4u_t size1 = LEN-size2; thread->last_pkt->double_access_vec = 1; mem_load_vector_oddva(thread, insn, EA+size1, EA+fVECSIZE(), 1, size2, &DST.ub[size1], size2, fUSE_LOOKUP_ADDRESS()); mem_load_vector_oddva(thread, insn, EA, EA, 0, size1, &DST.ub[0], size1, fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); }
#ifdef QEMU_GENERATE
...
#endif
#define fSTOREMMV_AL(EA,ALIGNMENT,LEN,SRC) { fV_AL_CHECK(EA,ALIGNMENT-1); mem_store_vector_oddva(thread, insn, EA&~(ALIGNMENT-1), EA, insn->slot, LEN, SRC.ub, 0, 0, fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); }
#ifdef QEMU_GENERATE
...
#endif
#define fSTOREMMVQ_AL(EA,ALIGNMENT,LEN,SRC,MASK) do { mmvector_t maskvec; int i; for (i = 0; i < fVECSIZE(); i++) maskvec.ub[i] = fGETQBIT(MASK,i); mem_store_vector_oddva(thread, insn, EA&~(ALIGNMENT-1), EA, insn->slot, LEN, SRC.ub, &maskvec.ub[0], 0, fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); } while (0)
#ifdef QEMU_GENERATE
...
#endif
#define fSTOREMMVNQ_AL(EA,ALIGNMENT,LEN,SRC,MASK) { mmvector_t maskvec; int i; for (i = 0; i < fVECSIZE(); i++) maskvec.ub[i] = fGETQBIT(MASK,i); fV_AL_CHECK(EA,ALIGNMENT-1); mem_store_vector_oddva(thread, insn, EA&~(ALIGNMENT-1), EA, insn->slot, LEN, SRC.ub, &maskvec.ub[0], 1, fUSE_LOOKUP_ADDRESS_BY_REV(thread->processor_ptr)); }
#ifdef QEMU_GENERATE
