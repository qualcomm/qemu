...
[ $# -ge 1 ] || die "usage: branch [git-publish options]...
...
if [ "$PULL_REQUEST" == "1" ]; then
    ...
    for p in $patches/*; do
        if ! grep -q -i 'Reviewed-by:' $p > /dev/null; then
            echo ERROR: $p missing 'Reviewed-by: (ignore with SKIP_REVIEW=1)'
...
{
    echo "CI for $branch:"
    echo "- https://github.com/qualcomm/qemu/tree/$branch-github-ci"
