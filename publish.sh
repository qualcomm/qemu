...
git fetch -a upstream || die "missing 'upstream' remote (git remote add upstream https://gitlab.com/qemu-project/qemu)"
...
# fetch CI
git fetch -a qemu-ci || die "missing 'qemu-ci' remote (git remote add qemu-ci https://github.com/p-b-o/qemu-ci)"
...
if [ "$PULL_REQUEST" == "1" ]; then
    ...
    git remote get-url gitlab || die "missing 'gitlab' remote (git remote add gitlab git@gitlab.com:<YOUR_USER>/qemu.git)"
